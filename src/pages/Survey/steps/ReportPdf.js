...
// import logo from '../../../assets/images/Bounce-Bar-Preloader'
import logo from '../../../assets/images/Bounce-Bar-Preloader.gif'

export default function ReportPdf(props) {
    // var param_id = window.location.search.split("id=")[1]
    // var param_company_id = window.location.search.split("company_id=")[1]

    // console.log(param_id);
    // console.log(param_company_id);
    const url_params = new URLSearchParams(window.location.search)
    const param_id = (url_params.get("id"));
    const company_id = (url_params.get("company_id"));
    ...
    const checkUserHosting = async (hostEmail, callback) => {
        let managersData = await fetch(`http://localhost:9000/masters/company/managers/${company_id}`)
    ...
    const ManagersLength = async () => {
        ...
        fetch(`http://localhost:9000/masters/company/managers/${company_id}`, requestOptions4)
    ...
    const GetAllRecords = async () => {
        ...
        fetch(`http://localhost:9000/masters/collect_feedback/${param_id}`, requestOptions)
            .then(response => response.json())
            .then(result => {
    ...
    const GetSurveyFeedRec = async () => {
        ...
        console.log(uid)
        fetch(`http://localhost:9000/masters/survey_feedback/company/${company_id}`, requestOptions)
            .then(response => response.json())
            .then(result => {
        ...
        fetch(`http://localhost:9000/masters/survey_feedback/${param_id}`, requestOptions)
            .then(response => response.json())
            .then(result => {
                // setlistRecord(result.data);
                console.log(result)
        ...
        <>
            ...
            {DisplayDiv && list1.length == 5 ? (
                <>
                    ...
                    {feedbackData ? (
                        <>
                            <div className=" row page-break feed_block_row" style={{ padding: "25px 25px 25px 25px" }} >
                                <div >
